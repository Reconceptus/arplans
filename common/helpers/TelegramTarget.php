...
class TelegramTarget extends Target
{
     ...
     */
    public $logFile='@common/runtime/logs/app.log';
    /**
    ...
    {
        $isNeedToSend = true;
        foreach ($this->messagesToRemove as $item){
            if(mb_strpos($text,$item)!==false){
                $isNeedToSend = false;
            }
        }
        if($isNeedToSend) {
            if(Telegram::sendFile($this->logFile)){
    ...
    /**
     * Ошибки, которые не надо отправлять сразу в телеграм, но они должны писаться в логи
     * @var string[]
     */
