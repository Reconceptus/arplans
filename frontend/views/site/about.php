<?php

/* @var $this yii\web\View */

$this->title = 'О нас';
$this->params['breadcrumbs'][] = $this->title;
?>
    <div class="section about--head">
        <div class="content content--lg mobile-wide">
            <div class="about--wrap gradient">
                <div class="content content--sm">
                    <h1 class="title title-lg">О нас</h1>
                    <h2 class="subtitle">Мы больше чем архитекторы зданий! Мы архитекторы вашей новой жизни в доме вашей
                        мечты! Воспользуйтесь нашим опытом для шага вперед. </h2>
                </div>
                <div class="about--image">
                    <div class="content content--md">
                        <figure>
                            <img src="assets/images/items/item10.jpg" alt="villa">
                        </figure>
                    </div>
                    <img src="assets/images/elements/branch2.png" alt="branch" class="img-branch">
                </div>
            </div>
        </div>
    </div>

    <div class="section custom-list">
        <div class="content content--md">
            <ul class="col-3">
                <li>
                    <div class="title">10 лет практики</div>
                    <p>Большая база готовых проектов домов и коттеджей. Мы регулярно пополняем наш каталог, еженедельно
                        обновляя его.</p>
                </li>
                <li>
                    <div class="title">Понятная документация</div>
                    <p>По нашим проектам может построить дом не только опытный строитель, но и любой желающий воплотить
                        мечту своими руками.</p>
                </li>
                <li>
                    <div class="title">Мы на связи 24/7/365</div>
                    <p>Работаем для Вас 24 часа в сутки 7 дней в неделю! Осуществляем поддержку до и после приобретения
                        нашего проекта</p>
                </li>
                <li>
                    <div class="title">Доступные цены</div>
                    <p>Мы проектировщики, а не перекупы, поэтому наши приятны и мы консультируем и после покупки.</p>
                </li>
                <li>
                    <div class="title">Бесплатные изменения</div>
                    <p>Изменить материал стен, отзеркалим проект, внесем небольшие коррективы — у Вас будет такой дом,
                        какой вы пожелаете.</p>
                </li>
                <li>
                    <div class="title">Гарантия окончательной стоимости</div>
                    <p>Текст. Ваш поселок увидят как те, кто планирует строить дом или переезд </p>
                </li>
                <li>
                    <div class="title">Мы знаем все о домах из дерева</div>
                    <p>Наши клиенты особенно полюбили наши проекты из дерева. В наших проектах обрезки материалов
                        сведены к минимуму. Вам не придется переплачивать лишние деньги! </p>
                </li>
                <li>
                    <div class="title">Посоветуем строителей</div>
                    <p>Одни лучше строят каменные дома, другие деревянные, мы посоветуем Вам лучшего застройщика под
                        конкретный проект!</p>
                </li>
                <li>
                    <div class="title">Корректируем фунтамент</div>
                    <p>Если у Вас есть геологические изыскания, при покупке проекта мы адаптируем фундамент под Ваш
                        грунт.</p>
                </li>
            </ul>
        </div>
    </div>

    <div class="big-header">
        <div class="content content--lg">
            <h2 class="title">РЕАЛИЗАЦИЯ НАШИХ ПРОЕКТОВ</h2>
        </div>
    </div>

    <div class="our-projects">
        <img src="assets/images/elements/branch3.png" alt="branch" class="img-branch">
        <div class="content content--md">
            <div class="our-projects-slider" data-owl="objects">
                <ul class="owl-carousel">
                    <li class="object-item">
                        <div class="projects-item--wrap">
                            <a href="#" class="projects-item--preview">
                                <div class="bg" style="background-image: url('assets/images/items/item01.jpg')"></div>
                                <div class="data">
                                    <div class="data-details">
                                        <span class="index">K-232</span>
                                        <span class="cost">Стоимость проекта: 25 000 &#8381;</span>
                                    </div>
                                    <ul class="info">
                                        <li>
                                            <span class="head">Жилая</span>
                                            <span>82 м<sup>2</sup></span>
                                        </li>
                                        <li>
                                            <span class="head">Полезная</span>
                                            <span>140 м<sup>2</sup></span>
                                        </li>
                                        <li>
                                            <span class="head">Общая</span>
                                            <span>260 м<sup>2</sup></span>
                                        </li>
                                    </ul>
                                </div>
                            </a>
                        </div>
                    </li>
                    <li class="object-item">
                        <div class="projects-item--wrap">
                            <a href="#" class="projects-item--preview">
                                <div class="bg" style="background-image: url('assets/images/items/item02.jpg')"></div>
                                <div class="data">
                                    <div class="data-details">
                                        <span class="index">K-232</span>
                                        <span class="cost">Стоимость проекта: 25 000 &#8381;</span>
                                    </div>
                                    <ul class="info">
                                        <li>
                                            <span class="head">Жилая</span>
                                            <span>82 м<sup>2</sup></span>
                                        </li>
                                        <li>
                                            <span class="head">Полезная</span>
                                            <span>140 м<sup>2</sup></span>
                                        </li>
                                        <li>
                                            <span class="head">Общая</span>
                                            <span>260 м<sup>2</sup></span>
                                        </li>
                                    </ul>
                                </div>
                            </a>
                        </div>
                    </li>
                </ul>
            </div>
            <div class="project-page--reviews">
                <i class="icon icon-vk-reviews">
                    <svg xmlns="http://www.w3.org/2000/svg">
                        <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-vk-reviews"/>
                    </svg>
                </i>
                <span class="text">Живые отзывы клиентов Вконтакте</span>
                <a href="#" class="read">Читать</a>
            </div>
        </div>
    </div>

    <div class="reviews">
        <div class="content content--md">
            <div class="reviews-carousel--box">
                <div class="reviews-carousel--wrap">
                    <div class="reviews-carousel" data-owl="reviews">
                        <ul class="owl-carousel">
                            <li class="item">
                                <div class="review-header">
                                    <div class="author">
                                        <div class="name">Владимир Иванов</div>
                                        <div class="position">Партнер</div>
                                    </div>
                                    <a href="#" class="load">PDF отзыв</a>
                                </div>
                                <div class="review-text">
                                    <p>«...Связался по телефону с сотрудником, помогли выбрать, оплатил картой, проект
                                        принес курьер черз 4 дня, домой, удобно. Уже начали строительство и даже на
                                        этапе фундамента нам уже нравится наш дом, все как мы хотели. Рады, что
                                        обратились к специалисту, а не сами придумывали»</p>
                                </div>
                            </li>
                            <li class="item">
                                <div class="review-header">
                                    <div class="author">
                                        <div class="name">Мария Иванова</div>
                                        <div class="position">Партнер</div>
                                    </div>
                                    <a href="#" class="load">PDF отзыв</a>
                                </div>
                                <div class="review-text">
                                    <p>«Мы очень довольны нашим домом! Построили за 4 месяца, все это время наш
                                        переодически консультировали сотрудники Арпланс. Очень хороший опыт, советую
                                        всем!»</p>
                                </div>
                            </li>
                            <li class="item">
                                <div class="review-header">
                                    <div class="author">
                                        <div class="name">Иван Иванов</div>
                                        <div class="position">Партнер</div>
                                    </div>
                                    <a href="#" class="load">PDF отзыв</a>
                                </div>
                                <div class="review-text">
                                    <p>«...Связался по телефону с сотрудником, помогли выбрать, оплатил картой, проект
                                        принес курьер черз 4 дня, домой, удобно. Уже начали строительство и даже на
                                        этапе фундамента нам уже нравится наш дом, все как мы хотели. Рады, что
                                        обратились к специалисту, а не сами придумывали»</p>
                                </div>
                            </li>
                            <li class="item">
                                <div class="review-header">
                                    <div class="author">
                                        <div class="name">Лёша Иванова</div>
                                        <div class="position">Партнер</div>
                                    </div>
                                    <a href="#" class="load">PDF отзыв</a>
                                </div>
                                <div class="review-text">
                                    <p>«Мы очень довольны нашим домом! Построили за 4 месяца, все это время наш
                                        переодически консультировали сотрудники Арпланс. Очень хороший опыт, советую
                                        всем!»</p>
                                </div>
                            </li>
                            <li class="item">
                                <div class="review-header">
                                    <div class="author">
                                        <div class="name">Владимир Иванов</div>
                                        <div class="position">Партнер</div>
                                    </div>
                                    <a href="#" class="load">PDF отзыв</a>
                                </div>
                                <div class="review-text">
                                    <p>«...Связался по телефону с сотрудником, помогли выбрать, оплатил картой, проект
                                        принес курьер черз 4 дня, домой, удобно. Уже начали строительство и даже на
                                        этапе фундамента нам уже нравится наш дом, все как мы хотели. Рады, что
                                        обратились к специалисту, а не сами придумывали»</p>
                                </div>
                            </li>
                            <li class="item">
                                <div class="review-header">
                                    <div class="author">
                                        <div class="name">Мария Иванова</div>
                                        <div class="position">Партнер</div>
                                    </div>
                                    <a href="#" class="load">PDF отзыв</a>
                                </div>
                                <div class="review-text">
                                    <p>«Мы очень довольны нашим домом! Построили за 4 месяца, все это время наш
                                        переодически консультировали сотрудники Арпланс. Очень хороший опыт, советую
                                        всем!»</p>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <div class="temp-nav">
            <button class="temp-nav-prev" id="t_prev"></button>
            <button class="temp-nav-next" id="t_next"></button>
        </div>
    </div>

    <div class="big-header">
        <div class="content content--lg">
            <h2 class="title">Офисы продаж</h2>
        </div>
    </div>
    <div class="map-box">
        <div class="content content--lg">
            <div class="map-box--wrap">
                <div class="map-box--main">
                    <div class="custom-search">
                        <form action="#">
                            <div class="custom-search--field">
                                <div class="custom-search--inputs">
                                    <div class="input region-dropbox">
                                        <input type="text" placeholder="Введите название населенного пункта">
                                        <div class="regions-drop">
                                            <ul>
                                                <li><a href="#">Москва</a></li>
                                                <li><a href="#">Санкт-Петербург</a></li>
                                                <li><a href="#">Московская область</a></li>
                                                <li><a href="#">Ханты-Мансийский автономный округ - Югра</a></li>
                                                <li><a href="#">Краснодарский край</a></li>
                                                <li><a href="#">Свердловская область</a></li>
                                                <li><a href="#">Ямало-Ненецкий автономный округ</a></li>
                                                <li><a href="#">Республика Татарстан</a></li>
                                                <li><a href="#">Красноярский край </a></li>
                                                <li><a href="#">Республика Башкортостан</a></li>
                                                <li><a href="#">Самарская область</a></li>
                                                <li><a href="#">Ростовская область</a></li>
                                                <li><a href="#">Челябинская область</a></li>
                                                <li><a href="#">Нижегородская область</a></li>
                                                <li><a href="#">Пермский край</a></li>
                                                <li><a href="#">Новосибирская область</a></li>
                                                <li><a href="#">Иркутская область</a></li>
                                                <li><a href="#">Тюменская область</a></li>
                                                <li><a href="#">Ленинградская область</a></li>
                                                <li><a href="#">Республика Саха (Якутия)</a></li>
                                                <li><a href="#">Кемеровская область</a></li>
                                                <li><a href="#">Воронежская область</a></li>
                                                <li><a href="#">Оренбургская область</a></li>
                                                <li><a href="#">Сахалинская область</a></li>
                                                <li><a href="#">Волгоградская область</a></li>
                                                <li><a href="#">Приморский край</a></li>
                                                <li><a href="#">Белгородская область</a></li>
                                                <li><a href="#">Саратовская область</a></li>
                                                <li><a href="#">Ставропольский край</a></li>
                                                <li><a href="#">Хабаровский край</a></li>
                                                <li><a href="#">Омская область</a></li>
                                                <li><a href="#">Республика Дагестан</a></li>
                                                <li><a href="#">Республика Коми</a></li>
                                                <li><a href="#">Удмуртская Республика</a></li>
                                                <li><a href="#">Тульская область</a></li>
                                                <li><a href="#">Алтайский край</a></li>
                                                <li><a href="#">Томская область</a></li>
                                                <li><a href="#">Вологодская область</a></li>
                                                <li><a href="#">Липецкая область</a></li>
                                                <li><a href="#">Ярославская область</a></li>
                                                <li><a href="#">Архангельская область</a></li>
                                                <li><a href="#">Мурманская область</a></li>
                                                <li><a href="#">Владимирская область</a></li>
                                                <li><a href="#">Калининградская область</a></li>
                                                <li><a href="#">Калужская область</a></li>
                                                <li><a href="#">Курская область</a></li>
                                                <li><a href="#">Тверская область</a></li>
                                                <li><a href="#">Астраханская область</a></li>
                                                <li><a href="#">Пензенская область</a></li>
                                                <li><a href="#">Рязанская область</a></li>
                                                <li><a href="#">Ульяновская область</a></li>
                                                <li><a href="#">Республика Крым </a></li>
                                                <li><a href="#">Петербург</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <button class="submit">
                                        <svg xmlns="http://www.w3.org/2000/svg">
                                            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-search"/>
                                        </svg>
                                    </button>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="map-box--field">
                        <div data-map="offices-map">
                            <div id="map"></div>
                            <script type="text/javascript">
                                var map;

                                function initMap() {

                                    var icon = './assets/images/map-mark.png';

                                    var mapOptions = {
                                        zoom: 10,
                                        mapTypeControl: false,
                                        streetViewControl: false,
                                        styles: [{
                                            "featureType": "water",
                                            "elementType": "geometry",
                                            "stylers": [{"color": "#e9e9e9"}, {"lightness": 17}]
                                        }, {
                                            "featureType": "landscape",
                                            "elementType": "geometry",
                                            "stylers": [{"color": "#f5f5f5"}, {"lightness": 20}]
                                        }, {
                                            "featureType": "road.highway",
                                            "elementType": "geometry.fill",
                                            "stylers": [{"color": "#ffffff"}, {"lightness": 17}]
                                        }, {
                                            "featureType": "road.highway",
                                            "elementType": "geometry.stroke",
                                            "stylers": [{"color": "#ffffff"}, {"lightness": 29}, {"weight": 0.2}]
                                        }, {
                                            "featureType": "road.arterial",
                                            "elementType": "geometry",
                                            "stylers": [{"color": "#ffffff"}, {"lightness": 18}]
                                        }, {
                                            "featureType": "road.local",
                                            "elementType": "geometry",
                                            "stylers": [{"color": "#ffffff"}, {"lightness": 16}]
                                        }, {
                                            "featureType": "poi",
                                            "elementType": "geometry",
                                            "stylers": [{"color": "#f5f5f5"}, {"lightness": 21}]
                                        }, {
                                            "featureType": "poi.park",
                                            "elementType": "geometry",
                                            "stylers": [{"color": "#dedede"}, {"lightness": 21}]
                                        }, {
                                            "elementType": "labels.text.stroke",
                                            "stylers": [{"visibility": "on"}, {"color": "#ffffff"}, {"lightness": 16}]
                                        }, {
                                            "elementType": "labels.text.fill",
                                            "stylers": [{"saturation": 36}, {"color": "#333333"}, {"lightness": 40}]
                                        }, {
                                            "elementType": "labels.icon",
                                            "stylers": [{"visibility": "off"}]
                                        }, {
                                            "featureType": "transit",
                                            "elementType": "geometry",
                                            "stylers": [{"color": "#f2f2f2"}, {"lightness": 19}]
                                        }, {
                                            "featureType": "administrative",
                                            "elementType": "geometry.fill",
                                            "stylers": [{"color": "#fefefe"}, {"lightness": 20}]
                                        }, {
                                            "featureType": "administrative",
                                            "elementType": "geometry.stroke",
                                            "stylers": [{"color": "#fefefe"}, {"lightness": 17}, {"weight": 1.2}]
                                        }],
                                        scrollwheel: false,
                                        center: new google.maps.LatLng(55.754080, 37.619151)
                                    };
                                    map = new google.maps.Map(document.getElementById('map'), mapOptions);

                                    var locations = [
                                        {lat: 55.754080, lng: 37.619151},
                                        {lat: 55.846140, lng: 37.849396}
                                    ];

                                    var marker;

                                    for (var i = 0; i < locations.length; i++) {
                                        marker = new google.maps.Marker({
                                            position: locations[i],
                                            map: map,
                                            icon: icon
                                        });
                                    }
                                }
                            </script>
                            <!-- API KEY for ARPLANS:  AIzaSyDTe4a3uDvnMx11fyEgPXA6UEEsNYh5Eg8-->
                            <script async defer
                                    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyC6PFq1z3G7_YGiZl1KUuVVH_kxI2YAdaA&callback=initMap"></script>

                        </div>
                    </div>
                </div>
                <div class="map-box--aside">
                    <div class="head">Офисы продаж в Нижнем Новгороде</div>
                    <div class="addresses scrolled">
                        <div class="item">
                            <div class="name">Планета Сервис</div>
                            <div>г. Коломна, ул.Рионская 55, оф. 303</div>
                            <div>+7 902 651-11-15</div>
                            <a href="#" class="on-map">на карте</a>
                        </div>
                        <div class="item">
                            <div class="name">СтроимВместе</div>
                            <div>г. Нижний Новгород, ул. Симбирская 33/1, левое крыло, оф. 303</div>
                            <div>+7 902 651-11-15</div>
                            <a href="#" class="on-map">на карте</a>
                        </div>
                        <div class="item">
                            <div class="name">Delsongo</div>
                            <div>г. Ростов Великий, ул.Рионская 55, оф. 303</div>
                            <div>+7 902 651-11-15</div>
                            <a href="#" class="on-map">на карте</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?= \frontend\widgets\recently\Recently::widget() ?>