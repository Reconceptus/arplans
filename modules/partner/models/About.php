...
class About extends Model
{
    ...
    public static function getFilteredQuery(array $get)
    {
        ...
        $query1 = Village::find()->alias('v')->select(['id', 'name', 'address', 'phones', 'email', 'url', 'lat', 'lng'])->where(['is_office' => 1, 'is_active' => 1]);
        $query2 = Builder::find()->alias('b')->select(['id', 'name', 'address', 'phones', 'email', 'url', 'lat', 'lng'])->where(['is_office' => 1, 'is_active' => 1]);
        if (isset($get['region'])) {
